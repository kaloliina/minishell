...
int	set_cmd_node(t_ast *ast, int i, int j, t_node *new_node)
{
	...
	if (ast->tokens[i][j + 1])
	{
		...
		while (k <= args)
		{
			if (!ft_strcmp(ast->tokens[i][init_j], "<<"))
				init_j++;
			else if (is_redirection(ast->tokens[i][init_j]))
				init_j += 2;
			else
			{
				new_node->cmd[k] = ast->tokens[i][init_j];
...
int	make_node_2(t_ast *ast, int i, int j, t_node **first)
{
	t_node	*new_node;
	t_node	*current;
...
int	lexer(t_ast *ast)
{
	...
	while (ast->sections[i]->section)
	{
		...
		while (ast->tokens[i][j])
		{
			if (is_redirection(ast->tokens[i][j]) && !redir_flag)
			{
				make_node(ast, i, j, &ast->first);
				redir_flag = 1;
			}
			j++;
		}
		...
		while (ast->tokens[i][j])
		{
			temp = make_node_2(ast, i, j, &ast->first);
			if (temp < 0)
				return (-1);
			j = temp;
		}
